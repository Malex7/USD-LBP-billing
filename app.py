...
# --- Exchange Box ---
with stylable_container(key="exchange_box"):
    st.markdown("""
        <style>
...
# --- Currency Box ---
with stylable_container(key="currency_box"):
    st.markdown("""
        <style>
        [data-testid="stVerticalBlock"] > div:has([data-testid="stSelectbox"]) {
            padding: 1rem;
...
# --- Payment Box ---
with stylable_container(key="payment_box"):
    st.markdown("""
        <style>
        [data-testid="stVerticalBlock"] > div:has([data-testid="stNumberInput"]) {
            padding: 1rem;
...
if st.button(TEXT["calculate"][lang]):
    ...
    with stylable_container(key="result_box"):
        st.markdown("""
            <style>
    ...
    if split_people > 0:
        ...
        with stylable_container(key="person_breakdown_box"):
            st.markdown("""
                <style>
                [data-testid="stVerticalBlock"] > div:has(h3) {
                    padding: 1rem;
